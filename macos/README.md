...
`TranscriptDraft` is pure Foundation and checked on Linux, not because the field is hard
to draw but because **its most important rule is invisible in a screenshot**: once the
user types, the ASR must stop writing to the field.

A final transcript can land *after* the field is on screen. A view bound straight to
`SessionModel.transcript` would silently discard the correction the user just made — the
worst possible bug here, since the entire design rests on the submitted text being theirs.
So `SessionModel.transcript` (what the ASR said) and `draft` (what will be sent) are
...
Sources/VNRKit/        event vocabulary, session state, audio framing, service client
  ...
  TranscriptDraft.swift  the editable transcript, GO and Cancel (PLAN §7)
